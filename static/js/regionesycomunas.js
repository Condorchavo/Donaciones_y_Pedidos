...
window.onload = function () {
    const regionSelection = document.querySelector("#region"),
          comunaSelection = document.querySelector("#comuna");

    comunaSelection.disabled = true;

    let regiones = regionsInfo["regiones"];

    for (let i=0; i < regiones.length; i++){
        regionSelection.options[regionSelection.options.length] = new Option(regiones[i].nombre, regiones[i].id);
    }

    regionSelection.onchange = (e) => {
        if (e.target.value){
            comunaSelection.disabled = false;
            comunaSelection.length = 1;
            let comunas = regionsInfo.regiones[e.target.value - 1].comunas; 
            for (let i=0; i < comunas.length; i++){
                comunaSelection.options[comunaSelection.options.length] = new Option(comunas[i].nombre, comunas[i].id);
            }
        }
        else{
            comunaSelection.disabled = true;
